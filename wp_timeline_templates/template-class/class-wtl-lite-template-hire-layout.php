 ...
 */
class Wtl_Lite_Template_Hire_Layout {
	/**
	...
	public static function js( $wtl_settings ) {
		...
		<?php
		$contents = ob_get_contents(); //phpcs:enable
		ob_end_clean();
		return $contents;
	}
	...
	public static function get_default_icon( $wtl_settings ) {
		...
		if ( 'fontawesome' === $icon_type ) {
			...
			if ( $icon_img ) {
				$out = '<img src="' . esc_attr( $icon_img ) . '">';
			} else {
				$out = $defaut_icon;
			}
		} else {
			$out = '<i class="far fa-clock"></i>';
		}
		echo '<foreignObject x="0" y="0" width="80" height="80"><div class="wtlcircle" xmlns="http://www.w3.org/1999/xhtml">' . wp_kses( $out, Wp_Timeline_Lite_Public::args_kses() ) . '</div></foreignobject>';
	}
		...
		<div class="wtl_progress-circle">
			<svg class="wtl-progress wtl_blue wtl_noselect" data-progress="<?php echo esc_attr( $progress ); ?>" x="0px" y="0px" viewBox="0 0 80 80" xmlns="http://www.w3.org/2000/svg">
				...
				if ( $time ) {
					...
					echo esc_attr( self::get_default_icon( $wtl_settings ), Wp_Timeline_Lite_Public::args_kses() );
				}
				?>
			</svg>
		</div>
...
$wtl_template_hire_layout = new Wtl_Lite_Template_Hire_Layout();

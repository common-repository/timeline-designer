 ...
 */
class Wtl_Lite_Custom_Post_Type {
	/**
		...
		if ( isset( $_GET['page'] ) && 'add_wtl_cpt' === $_GET['page'] ) {
			...
			if ( isset( $_REQUEST['savedata'] ) ) {
				...
				if ( ! isset( $_POST['wtl-cpt-form-submit-nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['wtl-cpt-form-submit-nonce'] ) ), 'wtl-cpt-form-submit' ) ) {
					esc_html_e( 'Sorry, your nonce did not verify.', 'timeline-designer' );
				} else {
					...
					if ( '' == $cpts_plural ) {
						wp_die( esc_html_e( 'Custom Post Type Plural Name is required.', 'timeline-designer' ), '', array( 'back_link' => true ) );
					}
					if ( '' == $cpts_singular ) {
						wp_die( esc_html_e( 'Custom Post Type Singular Name is required.', 'timeline-designer' ), '', array( 'back_link' => true ) );
					}
					if ( '' == $cpts_slug ) {
						wp_die( esc_html_e( 'Custom Post Type Slug is required.', 'timeline-designer' ), '', array( 'back_link' => true ) );
					}
					if ( 1 == $cpts_category && '' == $cpts_category_slug ) {
						wp_die( esc_html_e( 'Custom Category Slug is required.', 'timeline-designer' ), '', array( 'back_link' => true ) );
					}
					if ( 1 == $cpts_tag && '' == $cpts_tag_slug ) {
						wp_die( esc_html_e( 'Custom Tag Slug is required.', 'timeline-designer' ), '', array( 'back_link' => true ) );
					}
	...
	public function delete() {
		if ( isset( $_GET['page'] ) && 'wtl_cpts' === $_GET['page'] && isset( $_GET['action'] ) && 'delete' === $_GET['action'] && isset( $_GET['id'] ) && '' !== $_GET['id'] ) {
			if ( isset( $_GET['_wpnonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_GET['_wpnonce'] ) ) ) ) {
				global $wpdb;
				$id   = intval( $_GET['id'] );
				$data = $wpdb->get_row( $wpdb->prepare( 'SELECT slug,name FROM ' . $wpdb->prefix . 'wtl_cpts WHERE id = %d', $id ) );
				if ( ! empty( $data ) ) {
					$qry_args  = array(
						'post_type'      => $data->slug,
						'posts_per_page' => -1,
	...
	public function create_custom_post_type() {
		global $wpdb;
		if ( $wpdb->get_var( "SHOW TABLES LIKE '" . $wpdb->prefix . "wtl_cpts'" ) != $wpdb->prefix . 'wtl_cpts' ) {
			Wp_Timeline_Lite_Admin::wtl_create_cpts_table();
		}
		...
		if ( ! empty( $datas ) ) {
			foreach ( $datas as $data ) {
				...
				$cpts_labels  = array(
					...
					'add_new'            => esc_html__( 'Add New', 'timeline-designer' ),
					'add_new_item'       => esc_html__( 'Add New', 'timeline-designer' ) . sprintf( ' %s', $data_setting['cpts_singular'] ),
					'new_item'           => esc_html__( 'New', 'timeline-designer' ) . sprintf( ' %s', $data_setting['cpts_singular'] ),
					'edit_item'          => esc_html__( 'Edit', 'timeline-designer' ) . sprintf( ' %s', $data_setting['cpts_singular'] ),
					'view_item'          => esc_html__( 'View', 'timeline-designer' ) . sprintf( ' %s', $data_setting['cpts_singular'] ),
					'all_items'          => esc_html__( 'All', 'timeline-designer' ) . sprintf( ' %s', $data['name'] ),
					'search_items'       => esc_html__( 'Search', 'timeline-designer' ) . sprintf( ' %s', $data['name'] ),
					'parent_item_colon'  => esc_html__( 'Parent', 'timeline-designer' ) . sprintf( ' %s', $data['name'] ),
					'not_found'          => esc_html__( 'No', 'timeline-designer' ) . sprintf( ' %s ', $data['name'] ) . esc_html__( 'found', 'timeline-designer' ),
					'not_found_in_trash' => esc_html__( 'No', 'timeline-designer' ) . sprintf( ' %s ', $data['name'] ) . esc_html__( 'found in Trash', 'timeline-designer' ),
				);
				...
				if ( 1 == $data_setting['cpts_category'] ) {
					$category_name = ( '' !== $data_setting['cpts_category_name'] ) ? $data_setting['cpts_category_name'] : sprintf( '%s ', $data_setting['cpts_singular'] ) . esc_html__( 'Categories', 'timeline-designer' );
				...
				if ( 1 == $data_setting['cpts_tag'] ) {
					$tag_name = ( '' !== $data_setting['cpts_tag_name'] ) ? $data_setting['cpts_tag_name'] : sprintf( '%s ', $data_setting['cpts_singular'] ) . esc_html__( 'Tags', 'timeline-designer' );
	...
	public function wtl_multiple_delete_cpt() {
		...
		if ( isset( $_POST['wtl-cpt-form-list-nonce'] ) ) {
			if ( wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['wtl-cpt-form-list-nonce'] ) ), 'wtl-cpt-form-list' ) ) {
				if ( isset( $_POST['take_action'] ) && isset( $_GET['page'] ) && 'wtl_cpts' === $_GET['page'] ) {
					if ( ( isset( $_POST['wtl-action-top'] ) && 'delete_pr' === sanitize_text_field( wp_unslash( $_POST['wtl-action-top'] ) ) ) || ( isset( $_POST['wtl-action-top2'] ) && 'delete_pr' === sanitize_text_field( wp_unslash( $_POST['wtl-action-top2'] ) ) ) ) {
							...
							if ( is_array( $shortcodes ) ) {
								foreach ( $shortcodes as $shortcode ) {
									...
									if ( is_numeric( $shortcode ) ) {
										$get_data = $wpdb->get_row( $wpdb->prepare( 'SELECT * FROM ' . $wpdb->prefix . 'wtl_cpts WHERE id = %d', $shortcode ), ARRAY_A );
									}
									if ( ! empty( $get_data ) ) {
...
$wtl_custom_post_type = new Wtl_Lite_Custom_Post_Type();

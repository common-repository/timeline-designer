...
var wtlcpt={
    ...
    post_format:function(){
        var im=$('.wtl_single_image_format');
        var sf=$('.wtl_single_slideshow_format');
        var sv=$('.wtl_single_video_format');
        var sw=$('.wtl-single-video-type-wrap');
        var sq=$('.wtl_single_quote_format');
        var pf=$('#wtl_post_format');
        $('.buttonset').buttonset();
        im.hide();sf.hide();sv.hide();sw.hide();sq.hide();
        if(pf.val()=='image'){im.show()}else if(pf.val()=='slideshow'){sf.show()}else if(pf.val()=='video'){sv.show();sw.show()}else if(pf.val()=='quote'){sq.show()}
        pf.on('change',function(){var value=$(this).val();im.hide();sf.hide();sv.hide();sq.hide();sw.hide();if(value=='image'){im.show()}else if(value=='slideshow'){sf.show()}else if(value=='video'){sw.show();sv.show()}else if(value=='quote'){sq.show()}});
        ...
        oc.on('change',function(){var vl=$(this).val();of.hide();oi.hide();if(vl=='fontawesome'){of.show()}else if(vl=='image'){oi.show()}});
    },
    gallery_select:function(){
        $('#wtl_gallery_select').on('click', function (e) {
            ...
            file_frame.on('open', function () {
                var sl=file_frame.state().get('selection');var lb=file_frame.state('gallery-edit').get('library');var ids=$('#wtl_gallery_images').val();
                if(ids){var idsArray=ids.split(',');idsArray.forEach(function(id){var at=wp.media.attachment(id);at.fetch();sl.add(at?[at]:[])});file_frame.setState('gallery-edit');idsArray.forEach(function (id) {var at=wp.media.attachment(id);at.fetch();lb.add(at?[at]:[])})}
            });
            file_frame.on('update', function () {
                var ir = [];var imageHTML = '';var metadataString = '';
                var ig = file_frame.state().get('library');
                ...
                if(metadataString){$("#wtl_gallery_images").val(metadataString);$(".gallery_image_wrap ul").html(imageHTML);$('#wtl_gallery_select').text('Edit Selection');$('#wtl_gallery_removeall').removeClass('hidden')}
            });
            file_frame.open();
            var ir = [];var imageHTML = '';var metadataString = '';
                var ig = file_frame.state().get('library');
    ...
    gallery_remove:function(){
        $('#wtl_gallery_removeall').on('click', function (e) {
            ...
            if (confirm('Are you sure you want to remove all images?')) {
                $(".gallery_image_wrap ul").html("");$("#wtl_gallery_images").val("");$('#wtl_gallery_removeall').addClass('hidden');$('#wtl_gallery_select').text('Select Image');
            }
            e.stopImmediatePropagation();
        });
        $(document).on('click','.gallery_image_wrap ul li button',function(e){
            ...
            if (confirm(wtl_admin_cpts_translations.conformation)){
                ...
                if(ng==""){$('#wtl_gallery_select').text('Select Image');$('#wtl_gallery_removeall').removeClass('visible')}
            }
        });
    },
    video_change:function(){
        var pl=$('.wtl-post-link');
        $('.wtl_single_video_type').on('change',function(){
            wtlcpt.video_thumbnail_type();
            $('input[name="wtl_single_video_id"]').val('');
        });
    ...
    upload_image:function(){
        ...
        frame.on('select',function(){
            ...
            var ii='#wtl_icon_image_id',im='#wtl_icon_image_src';
            if(el.hasClass( "wtl_icon_image" )){
                pr.find(ii).val(attachment.attributes.id);pr.find(im).val(attachment.attributes.url);
            }else{
                pr.find(ii).val(attachment.attributes.id);pr.find(im).val(attachment.attributes.url);
            }
        ...
        $(document).on('click', '.wtl_remove_image_button', function(e){
            ...
            if (confirm('Are you sure you want to remove images?')) {
                ...
                if(el.hasClass( "wtl_icon_image" )){
                    ...
                    $('#wtl_icon_image_id').val('');$('#wtl_icon_image_src').val('');
    ...
    video_thumbnail_type:function(){
        $=jQuery;
        var vt = $('.wtl_single_video_type').val();
        var ws='.wtl-single-video-type-wrap ';
        var wd=ws+'.wtl-single-video-description';
        var wl=ws+'.wtl-single-label';
        if(vt=='youtube'){$(wd).text('Enter the ID of the Youtube video.');$(wl).text('Youtube video ID')}
        if(vt=='vimeo'){$(wd).text('Enter the ID of the Vimeo video.');$(wl).text('Vimeo video ID')}
        if(vt=='screenr'){$(wd).text('Enter the ID of the Screenr video.');$(wl).text('Screenr video ID')}
        if(vt=='dailymotion'){$(wd).text('Enter the ID of the Dailymotion video.');$(wl).text('Dailymotion video ID')}
        if(vt=='metacafe'){$(wd).text('Enter the ID and slug of the Metacafe video.');$(wl).text('Metacafe video ID')}
        if(vt=='html5'){$(wd).text('Enter the URL of the Html5 video.');$(wl).text('Html5 video url')}
    },
